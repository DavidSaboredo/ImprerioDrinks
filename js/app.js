(() => {
  ...
  const els = {
    ...
    adjustmentsSubtotal: $("#adjustmentsSubtotal"),
    ...
    downloadInternalPdf: $("#downloadInternalPdf"),
    copyQuote: $("#copyQuote"),
    reloadPrices: $("#reloadPrices"),
    shareHelp: $("#shareHelp"),
    toast: $("#toast"),
    internalCostsSection: $("#internalCostsSection"),
    internalCostAmount: $("#internalCostAmount"),
  ...
  const numberOf = (element) =>
    Number.isFinite(Number(element.value)) ? Number(element.value) : 0;
  const money = (value) => currency.format(Math.round(Number(value) || 0));
  const formatDate = (value) =>
    value ? dateFormatter.format(new Date(`${value}T12:00:00`)) : "A coordinar";
  ...
  function reloadConfig(showMessage = true) {
    ...
    selectedPlanId = config.plans.some(
      (plan) => plan.id === previousPlan && plan.active,
    )
      ? previousPlan
      : config.plans.find((plan) => plan.active)?.id || null;
    cart = cart.filter((item) =>
      config.products.some(
        (product) => product.id === item.productId && product.active,
      ),
    );
    const minimum = config.business.minimumGuests || 1;
    els.guestCount.min = String(minimum);
    if (numberOf(els.guestCount) < minimum)
      els.guestCount.value = String(minimum);
  ...
  function renderPlans() {
    ...
    if (!plans.length) {
      els.planGrid.innerHTML =
        '<div class="empty">No hay planes activos. Agregalos desde Administración.</div>';
      return;
    }
    ...
    plans.forEach((plan) => {
      ...
      const visibleIncludes = plan.includes
        .slice(0, 6)
        .map((item) => `<li>${escapeHtml(item)}</li>`)
        .join("");
      const remaining =
        plan.includes.length > 6
          ? `<li>+ ${plan.includes.length - 6} opciones más</li>`
          : "";
      card.innerHTML = `
  ...
  function renderProductSelect() {
    const activeProducts = config.products.filter(
      (product) => product.active && product.visible,
    );
  ...
  function renderCart() {
    ...
    if (!cart.length) {
      els.selectedProducts.innerHTML =
        '<div class="empty">No agregaste productos adicionales.</div>';
      return;
    }
  ...
  function quote() {
    const minimumGuests = Math.max(
      1,
      Number(config.business.minimumGuests) || 1,
    );
    const enteredGuests = Math.max(0, Math.floor(numberOf(els.guestCount)));
    const billableGuests = Math.max(
      minimumGuests,
      enteredGuests || minimumGuests,
    );
    const plan =
      config.plans.find(
        (candidate) => candidate.id === selectedPlanId && candidate.active,
      ) ||
    ...
    const { profit, margin } = ImperioCosts.calculateProfitMargin(
      total,
      internalCostsData.totalCost,
    );
  ...
  function update() {
    const data = quote();
    const belowMinimum =
      data.enteredGuests > 0 && data.enteredGuests < data.minimumGuests;
    ...
    els.summaryPeople.textContent =
      data.enteredGuests && data.enteredGuests !== data.billableGuests
    ...
    if (data.requestedManualDiscount > data.manualDiscount) {
      els.manualDiscount.title =
        "El descuento fue limitado para que el total no sea negativo.";
    } else {
  ...
  function buildMessage() {
    const data = quote();
    const included = data.plan?.includes?.length
      ? data.plan.includes.map((item) => `• ${item}`).join("\n")
      : "• A confirmar";
    const products = data.products.length
      ? data.products
          .map(
            (item) =>
              `• ${item.product.name}: ${item.qty} ${item.product.unit} — ${money(item.total)}`,
          )
          .join("\n")
  ...
  const approxWidth = (text, size, bold = false) =>
    cleanPdfText(text).length * size * (bold ? 0.56 : 0.51);
  ...
  function createPdfBytes(qrDataUrl) {
    ...
    const W = 595.28,
      ...
      M = 38;
    const COLORS = {
      dark: [0.01, 0.025, 0.03],
      cyan: [0.06, 0.76, 0.79],
      amber: [0.95, 0.56, 0.1],
      ink: [0.08, 0.1, 0.11],
      muted: [0.38, 0.44, 0.46],
      light: [0.965, 0.975, 0.975],
      border: [0.82, 0.86, 0.87],
      white: [1, 1, 1],
      green: [0.1, 0.55, 0.31],
      red: [0.7, 0.16, 0.13],
    };
    ...
    const rgb = (color) =>
      `${color.map((value) => Number(value).toFixed(3)).join(" ")} rg`;
    const strokeRgb = (color) =>
      `${color.map((value) => Number(value).toFixed(3)).join(" ")} RG`;
    ...
    const rectTop = (
      ...
      if (stroke)
        commands.push(
          `${strokeRgb(stroke)} ${lineWidth} w ${x} ${y} ${width} ${height} re S\n`,
        );
    };
    const lineTop = (x1, top1, x2, top2, color, lineWidth = 1) =>
      commands.push(
        `${strokeRgb(color)} ${lineWidth} w ${x1} ${yFromTop(top1)} m ${x2} ${yFromTop(top2)} l S\n`,
      );
    const textTop = (
      text,
      x,
      ...
      commands.push(
        `BT /${bold ? "F2" : "F1"} ${size} Tf ${rgb(color)} 1 0 0 1 ${x} ${baseline} Tm (${pdfEscape(text)}) Tj ET\n`,
      );
    };
    const rightTextTop = (
      text,
      ...
      textTop(
        text,
        rightX - approxWidth(text, size, bold),
        top,
        size,
        bold,
        color,
    ...
    function startPage(isContinuation = false) {
      ...
      textTop(
        cleanPdfText(config.business.name || "IMPERIO DRINKS").toUpperCase(),
        132,
        27,
        19,
        true,
      ...
      textTop(
        isContinuation
          ...
          : "PRESUPUESTO DE BEBIDAS PARA EVENTOS",
        132,
        54,
        9.5,
        true,
      ...
      textTop(
        cleanPdfText(config.business.name || "IMPERIO DRINKS"),
        M,
        811,
        8.2,
        true,
      ...
      const contacts = [
        config.business.phone ? `WhatsApp: ${config.business.phone}` : "",
        config.business.instagram || "",
        config.business.paymentAlias
          ? `Alias: ${config.business.paymentAlias}`
          : "",
      ]
        .filter(Boolean)
        .join(" | ");
      rightTextTop(
        cleanPdfText(contacts),
        W - M,
        812,
        7.1,
        false,
    ...
    function drawWrapped(
      ...
      lines.forEach((line) => {
        textTop(line, x, cursor, size, bold, color);
        cursor += lineHeight;
      });
      return lines.length;
    }
    ...
    const fields = [
      [
        "Cliente",
        els.clientName.value.trim() || "A confirmar",
        leftX,
        cursor + 12,
      ],
      [
        "WhatsApp",
        els.clientPhone.value.trim() || "A confirmar",
        rightX,
        cursor + 12,
      ],
      ...
      [
        "Lugar / duración",
        [els.eventLocation.value.trim(), els.serviceHours.value.trim()]
          .filter(Boolean)
          .join(" - ") || "A confirmar",
        leftX,
        cursor + 68,
    ...
    fields.forEach(([label, value, x, top]) => {
      ...
      textTop(
        cleanPdfText(value).slice(0, x === leftX ? 52 : 34),
        x,
        top + 11,
        9,
        label === "Cliente" || label === "Personas",
        COLORS.ink,
      );
    });
    cursor += 105;
    ...
    textTop(
      `${pdfMoney(data.planUnitPrice)} por persona × ${data.billableGuests} personas`,
      M + 15,
      cursor + 36,
      8.7,
      false,
    ...
    const columnData = columns.map((column) =>
      column.map((item) => wrapText(item, 44)),
    );
    const heights = columnData.map((column) =>
      column.reduce(
        (sum, lines) => sum + Math.max(15, lines.length * 10 + 4),
        0,
      ),
    );
    ...
    const detailRows = [
      {
        label: `${data.plan.name} - ${data.billableGuests} personas`,
        amount: data.planSubtotal,
      },
      ...data.products.map((item) => ({
        label: `${item.product.name} - ${item.qty} ${item.product.unit}`,
        amount: item.total,
      })),
      ...(data.travel > 0
        ? [{ label: "Traslado / logística", amount: data.travel }]
        : []),
      ...(data.other > 0
        ? [{ label: "Otros gastos", amount: data.other }]
        : []),
      ...(data.automaticDiscount > 0
        ? [
            {
              label: `Descuento por cantidad (${data.automaticDiscountPercent}%)`,
              amount: -data.automaticDiscount,
            },
          ]
        : []),
      ...(data.manualDiscount > 0
        ? [{ label: "Descuento manual", amount: -data.manualDiscount }]
        : []),
    ];
    ...
    detailRows.forEach((row, index) => {
      ...
      if (index % 2 === 0)
        rectTop(M, cursor, W - M * 2, 21, [0.982, 0.987, 0.987]);
      textTop(
        cleanPdfText(row.label).slice(0, 70),
        M + 10,
        cursor + 6,
        8.1,
        false,
      ...
      rightTextTop(
        `${row.amount < 0 ? "-" : ""}${pdfMoney(Math.abs(row.amount))}`,
        W - M - 10,
        cursor + 6,
        8.1,
        row.amount < 0,
        row.amount < 0 ? COLORS.red : COLORS.ink,
      );
    ...
    textTop("Saldo", M + 15, cursor + 61, 8.2, false, [0.8, 0.83, 0.84]);
    rightTextTop(
      pdfMoney(data.balance),
      W - M - 15,
      cursor + 59,
      10.5,
      true,
    ...
    if (els.notes.value.trim()) {
      ...
      noteLines.forEach((line) => {
        textTop(line, M, cursor, 8.0, false, COLORS.muted);
        cursor += 11;
      });
      cursor += 8;
    }
    ...
    pages.forEach((page, index) => {
      ...
      textTop(
        pageNumber,
        (W - approxWidth(pageNumber, 7.1, false)) / 2,
        811,
        7.1,
        false,
        COLORS.muted,
    ...
    objects[2] = latin1Bytes(
      `<< /Type /Pages /Kids [${pageIds.map((id) => `${id} 0 R`).join(" ")}] /Count ${pageCount} >>`,
    );
    pages.forEach((pageCommands, index) => {
      objects[pageIds[index]] = latin1Bytes(
        `<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${W} ${H}] /Resources << /Font << /F1 ${fontRegularId} 0 R /F2 ${fontBoldId} 0 R >> /XObject << /Im1 ${imageId} 0 R >> >> /Contents ${contentIds[index]} 0 R >>`,
      );
      const content = latin1Bytes(pageCommands.join(""));
      objects[contentIds[index]] = concatByteArrays([
        latin1Bytes(`<< /Length ${content.length} >>\nstream\n`),
        content,
        latin1Bytes("\nendstream"),
      ]);
    });
    objects[fontRegularId] = latin1Bytes(
      "<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>",
    );
    objects[fontBoldId] = latin1Bytes(
      "<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold /Encoding /WinAnsiEncoding >>",
    );
    objects[imageId] = concatByteArrays([
      latin1Bytes(
        `<< /Type /XObject /Subtype /Image /Width 1254 /Height 1254 /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${imageBytes.length} >>\nstream\n`,
      ),
      imageBytes,
      latin1Bytes("\nendstream"),
    ]);
  ...
  function createInternalPdfBytes(qrDataUrl) {
    ...
    function startPage(isContinuation = false) {
      ...
      textTop(
        "Planificación y control de márgenes",
        132,
        72,
        8.5,
        false,
      ...
      rightTextTop(
        `Preparado: ${dateFormatter.format(new Date())}`,
        W - M,
        29,
        8.1,
        false,
    ...
    const fields = [
      ...
      [
        "Duración",
        els.serviceHours.value.trim() || "A confirmar",
        rightX,
        cursor + 40,
      ],
    ...
    const consumptionRows = (data.internalCosts?.productDetails || []).map(
      (detail) => ({
        label: `${detail.name}`,
    ...
    if (consumptionRows.length > 0) {
      ...
      consumptionRows.forEach((row, index) => {
        ...
        if (index % 2 === 0)
          rectTop(M, cursor, W - M * 2, 21, [0.982, 0.987, 0.987]);
        textTop(
          cleanPdfText(row.label).slice(0, 50),
          M + 10,
          cursor + 6,
          8.1,
          row.isAdditional,
    ...
    rightTextTop(
      pdfMoney(data.internalCosts?.totalCost || 0),
      W - M - 15,
      cursor + 20,
      17.5,
      true,
    ...
    // 1 = catálogo
    // 2 = contenedor de páginas
    // Las páginas empiezan desde el objeto 3
    const pageIds = Array.from({ length: pageCount }, (_, index) => 3 + index);
    ...
    pages.forEach((pageCommands, index) => {
      objects[pageIds[index]] = latin1Bytes(
        `<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${W} ${H}] /Resources << /Font << /F1 ${fontRegularId} 0 R /F2 ${fontBoldId} 0 R >> >> /Contents ${contentIds[index]} 0 R >>`,
      );
      const content = latin1Bytes(pageCommands.join(""));
      objects[contentIds[index]] = concatByteArrays([
        latin1Bytes(`<< /Length ${content.length} >>\nstream\n`),
  ...
  function createInternalPdfFile(qrDataUrl) {
    ...
    return new File([bytes], safeFilenameInternal(), {
      type: "application/pdf",
      lastModified: Date.now(),
    });
  }
  ...
  function normalizedWhatsappPhone(raw) {
    ...
    if (!digits) return "";
    const countryCode =
      String(config.business.countryCode || "54").replace(/\D/g, "") || "54";
  ...
  function generateQrAndDownloadPdf() {
    try {
      const file = createPdfFile(null);
      downloadFile(file);
  ...
  function downloadInternalPdf() {
    try {
      ...
      showToast("Generando análisis interno...");
    } catch (error) {
      console.error(error);
  ...
  function generateQrAndDownloadInternalPdf() {
    try {
      const file = createInternalPdfFile(null);
      downloadFile(file);
      showToast("Análisis interno descargado");
    } catch (error) {
      console.error("Error al generar análisis interno:", error);
      showToast(error?.message || "No se pudo generar el análisis interno");
    }
  }
  ...
  window.addEventListener("storage", (event) => {
    if (event.key === ImperioStore.key) reloadConfig(false);
  });
